...
export function MissionWorkspace({ onCompletedChange }: WorkspaceProps) {
  ...
  if (screen === "start") return <section className="welcome" aria-labelledby="welcome-title"><p className="eyebrow">초등 5~6학년 · 약 15~20분</p><h1 id="welcome-title">빛은 어디에서 곧게 가고,<br />어디에서 방향을 바꿀까요?</h1><p>빛길을 먼저 예상하고, 거울과 렌즈의 역할을 고른 뒤 화면의 빛길을 관찰해 봐요.</p><div className="solve-guide"><h2>이렇게 풀어요</h2><ol><li>그림에서 광원·장치·표적을 찾아요.</li><li>빛이 어떻게 갈지 먼저 예상해요.</li><li>빛길을 이어 줄 장치를 골라요.</li><li>노란 선을 따라가며 본 것을 설명해요.</li></ol></div><div className="goal-list"><span>1. 먼저 예상하기</span><span>2. 빛길 관찰하기</span><span>3. 본 것을 설명하기</span></div><p className="privacy">이 앱은 이름이나 사진을 받지 않고, 선택은 이 화면을 닫으면 남지 않아요.</p><button className="primary-button gi-pulse" type="button" onClick={() => setScreen("safety")}><span className="now-label">지금 할 일 · </span>빛길 구조 시작 <span aria-hidden="true">→</span></button></section>;
  if (screen === "safety") return <section className="safety-screen" aria-labelledby="safety-title"><p className="eyebrow">시작 전 약속</p><h1 id="safety-title">이 화면은 실제 빛을 보여 주는 실험이 아니에요.</h1><div className="solve-guide"><h2>이렇게 풀어요</h2><p>화면의 노란 선을 처음부터 따라가며 빛이 멈추거나 꺾이는 곳을 찾아요.</p></div><div className="safety-grid"><article><h2>화면 속 빛줄기</h2><p>노란 선은 빛이 나아가는 방향을 보여 주는 선이에요.</p></article><article><h2>이 화면에서 볼 수 있는 것</h2><p>이 화면은 나란히 들어오는 세 빛줄기가 모이는 장면만 보여 줘요.</p></article><article><h2>안전하게</h2><p>강한 빛이나 햇빛을 눈에 비추거나 렌즈로 모으지 않아요. 실제 실험은 선생님과 해요.</p></article></div><button className="primary-button gi-pulse" type="button" onClick={() => setScreen("mission")}><span className="now-label">지금 할 일 · </span>확인했어요</button></section>;
  ...
  const phase = !sceneViewed ? "장면 찾기" : !prediction ? "예상" : !setup ? "장치" : !trace ? "장치 확인" : !canExplain ? "수정" : visibleSegments < trace.segments.length ? "관찰" : !explanationDone ? "설명" : "완료";
  const pulse = (target: string) => target === "장면 찾기" ? "" : phase === target ? " gi-pulse" : "";
  return <section className="workspace" aria-labelledby="mission-title"><div className="mission-topline"><span>{index === 0 ? "안내 활동" : `본 미션 ${index}/5`}</span><span>전체 6단계 · 현재: <strong>{phase}</strong></span></div><div className="progress-wrap"><span>장면 찾기 → 예상 → 장치 → 관찰 → 설명 → 완료</span><div role="progressbar" aria-label="활동 진행" aria-valuemin={1} aria-valuemax={MISSIONS.length} aria-valuenow={index + 1}><span style={{ width: `${((index + 1) / MISSIONS.length) * 100}%` }} /></div></div><h1 id="mission-title" tabIndex={-1} ref={headingRef}>{mission.title}</h1><p className="mission-request">{mission.request}</p><section className="current-step" aria-live="polite"><strong>지금 할 일</strong><span>{phase === "장면 찾기" ? "그림에서 광원·장치·표적을 먼저 찾아요." : phase === "예상" ? "빛이 어떻게 갈지 예상 하나를 골라요." : phase === "장치" ? "빛길을 이어 줄 장치 하나를 골라요." : phase === "관찰" ? "노란 선을 처음부터 따라가요." : phase === "수정" ? "장치 다시 고르기를 눌러 한 가지 조건을 바꿔요." : phase === "설명" ? "방금 본 장면과 같은 설명을 골라요." : phase === "완료" ? "다음 활동으로 가서 빛의 규칙을 정리해요." : "고른 장치로 빛길 확인을 눌러요."}</span></section><div className="workspace-grid"><LightPathScene ref={sceneRef} mission={mission} trace={trace} setupId={setup} visibleSegments={visibleSegments} /><div className="choice-column"><fieldset className={`choice-panel${pulse("장면 찾기")}`}><legend>1. 그림에서 먼저 찾아요</legend><p>광원 → 장치 → 표적 순서로 그림을 살펴본 뒤 표시해요.</p><div className="scene-find-list">{mission.sceneGuide.map((item) => <span key={item.label}><strong>{item.label}</strong> · {item.hint}</span>)}</div><button type="button" className={sceneViewed ? "secondary-action" : "primary-button gi-pulse"} onClick={() => setSceneViewed(true)} disabled={sceneViewed}>{sceneViewed ? "찾았어요 ✓" : "찾았어요"}</button></fieldset><fieldset className={`choice-panel${pulse("예상")}`}><legend>2. 내 예상은?</legend><p>결과를 보기 전에 하나를 골라요.</p><div className="choice-list">{mission.predictions.map((choice) => <label key={choice.id} className={prediction === choice.id ? "choice selected" : "choice"}><input type="radio" name="prediction" disabled={!sceneViewed} checked={prediction === choice.id} onChange={() => choosePrediction(choice.id)} /><span>{choice.label}{choice.detail && <small>{choice.detail}</small>}</span></label>)}</div></fieldset><fieldset className={`choice-panel${pulse("장치")}`}><legend>3. {mission.setupLabel}</legend><p>준비된 보기 중 하나를 골라요.</p><div className="choice-list">{mission.setups.map((choice) => <label key={choice.id} className={setup === choice.id ? "choice selected" : "choice"}><input type="radio" name="setup" disabled={!prediction} checked={setup === choice.id} onChange={() => changeSetup(choice.id)} /><span>{choice.label}{choice.detail && <small>{choice.detail}</small>}</span></label>)}</div></fieldset><button className={`primary-button${phase === "장치 확인" ? " gi-pulse" : ""}`} type="button" disabled={!prediction || !setup} onClick={checkTrace}><span className="now-label">지금 할 일 · </span>빛길 확인</button>{trace && <TraceRecord trace={trace} prediction={predictionChoice?.label ?? ""} predictionMatches={predictionMatchesObservation(mission, prediction ?? "", setup ?? "")} visibleSegments={visibleSegments} pulseNext={phase === "관찰"} onStep={() => setVisibleSegments((value) => Math.min(value + 1, trace.segments.length))} onShowAll={() => setVisibleSegments(trace.segments.length)} />}</div></div>{trace && <section className={`explanation-panel${pulse("설명")}`} aria-labelledby="explanation-title"><h2 id="explanation-title">4. 본 것을 바탕으로 설명하기</h2><p>빛이 멈추거나 방향을 바꾼 곳을 먼저 찾고, 빛길 순서를 읽은 뒤 알맞은 설명을 골라요.</p><p className="concept-help">{mission.conceptHelp}</p><div className="choice-list">{mission.explanations.map((choice) => <label key={choice.id} className="choice"><input type="radio" name="explanation" disabled={!canExplain} checked={explanation === choice.id} onChange={() => chooseExplanation(choice.id)} />{choice.label}</label>)}</div>{feedback && <p className={explanationDone ? "feedback success" : "feedback"} role="status">{feedback}</p>}{repairHint && <><p className="hint">{repairHint}</p><button type="button" className={phase === "수정" ? "secondary-action gi-pulse" : "secondary-action"} onClick={repairSetup}>장치 다시 고르기</button></>}{explanationDone && <button className={`primary-button${pulse("완료")}`} type="button" onClick={next}><span className="now-label">지금 할 일 · </span>{index === MISSIONS.length - 1 ? "관찰 기록 보기" : "다음 활동으로"}</button>}</section>}<aside className="model-note"><strong>모형 약속</strong><p>{mission.modelNote}</p><p>{mission.safetyNote}</p></aside></section>;
}

function TraceRecord({ trace, prediction, predictionMatches, visibleSegments, pulseNext, onStep, onShowAll }: { trace: TraceResult; prediction: string; predictionMatches: boolean; visibleSegments: number; pulseNext: boolean; onStep: () => void; onShowAll: () => void }) {
  const noLightPath = trace.segments.length === 0;
  return <section className="trace-record" aria-labelledby="trace-title"><h2 id="trace-title">빛길 순서</h2><p className="trace-progress" role="status">{noLightPath ? "빛길이 시작되지 않음" : `현재 ${visibleSegments}/${trace.segments.length} 구간`}</p><div className="compare-record"><p><strong>첫 예상</strong><span>{prediction}</span></p><p><strong>관찰 결과</strong><span>{trace.summary}</span></p></div><p className="comparison">{predictionMatches ? "예상과 관찰이 같았어요. 빛이 멈추거나 방향을 바꾼 곳을 찾아 설명해 봐요." : "예상과 관찰이 달랐어요. 빛이 멈추거나 방향을 바꾼 곳을 보고 다시 생각해 봐요."}</p>{!noLightPath && <><div className="trace-controls"><button className={pulseNext ? "gi-pulse" : ""} type="button" onClick={onStep} disabled={visibleSegments >= trace.segments.length}><span className="now-label">지금 할 일 · </span>다음 빛길 보기</button><button type="button" onClick={onShowAll}>빛길 한 번에 보기</button></div><ol>{trace.segments.slice(0, visibleSegments).map((segment) => <li key={segment.label}>{segment.label}</li>)}</ol></>}</section>;
}
