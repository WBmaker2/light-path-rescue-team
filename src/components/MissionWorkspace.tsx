...
import { mainMissionRecords } from "../domain/session-summary";
import type { MissionId, TraceResult } from "../domain/types";
import { LightPathScene } from "./LightPathScene";

type MissionRecord = { id: MissionId; prediction: string; setup: string; changed: boolean; attempts: number; trace: TraceResult; explanation: string };
type WorkspaceProps = { onCompletedChange: (count: number) => void };
const successPredictions = new Set(["visible", "straight", "reflect", "twice", "focus", "uses"]);
...
export function MissionWorkspace({ onCompletedChange }: WorkspaceProps) {
  ...
  const checkTrace = () => {
    ...
    setTrace(result); setAttempts((value) => value + 1); setFirstSetup((value) => value ?? setup); setVisibleSegments(reduceMotion ? result.segments.length : 1); setExplanation(null); setFeedback("");
  };
  ...
  if (screen === "summary") return <SessionSummary records={records} onRestart={() => { setScreen("start"); setIndex(0); setRecords([]); }} />;
  ...
  return <section className="workspace" aria-labelledby="mission-title"><div className="mission-topline"><span>{index === 0 ? "안내 활동" : `본 미션 ${index}/5`}</span><span>단계: 예상 → 선택 → 관찰 → 근거</span></div><h1 id="mission-title" tabIndex={-1} ref={headingRef}>{mission.title}</h1><p className="mission-request">{mission.request}</p><div className="workspace-grid"><div><fieldset className="choice-panel"><legend>1. 내 예상은?</legend><p>결과를 보기 전에 하나를 골라요.</p><div className="choice-list">{mission.predictions.map((choice) => <label key={choice.id} className={prediction === choice.id ? "choice selected" : "choice"}><input type="radio" name="prediction" checked={prediction === choice.id} onChange={() => choosePrediction(choice.id)} />{choice.label}</label>)}</div></fieldset><fieldset className="choice-panel"><legend>2. {mission.setupLabel}</legend><p>드래그하지 않고, 검수된 선택지 중 하나를 고를 수 있어요.</p><div className="choice-list">{mission.setups.map((choice) => <label key={choice.id} className={setup === choice.id ? "choice selected" : "choice"}><input type="radio" name="setup" checked={setup === choice.id} onChange={() => changeSetup(choice.id)} />{choice.label}</label>)}</div></fieldset><button className="primary-button" type="button" disabled={!prediction || !setup} onClick={checkTrace}>빛길 확인</button>{trace && <TraceRecord trace={trace} prediction={predictionChoice?.label ?? ""} predictionMatches={successPredictions.has(prediction ?? "") && trace.status === "target-hit"} visibleSegments={visibleSegments} onStep={() => setVisibleSegments((value) => Math.min(value + 1, trace.segments.length))} onShowAll={() => setVisibleSegments(trace.segments.length)} />}</div><LightPathScene mission={mission} trace={trace} setupId={setup} visibleSegments={visibleSegments} /></div>{trace && <section className="explanation-panel" aria-labelledby="explanation-title"><h2 id="explanation-title">3. 관찰 근거로 설명하기</h2><p><strong>어디에서?</strong> {mission.property}와 관련된 지점을 골라요. <strong>어떻게?</strong> 빛길이 바뀐 모습을 읽어요. <strong>그래서?</strong> 표적과 연결해요.</p><div className="choice-list">{mission.explanations.map((choice) => <label key={choice.id} className={explanation === choice.id ? "choice selected" : "choice"}><input type="radio" name="explanation" checked={explanation === choice.id} onChange={() => chooseExplanation(choice.id)} />{choice.label}</label>)}</div>{feedback && <p className={explanationDone ? "feedback success" : "feedback"} role="status">{feedback}</p>}{repairHint && <p className="hint">{repairHint}</p>}{explanationDone && <button className="primary-button" type="button" onClick={next}>{index === MISSIONS.length - 1 ? "관찰 기록 보기" : "다음 활동으로"}</button>}</section>}<aside className="model-note"><strong>모형 약속</strong><p>{mission.modelNote}</p><p>{mission.safetyNote}</p></aside></section>;
}
...
function TraceRecord({ trace, prediction, predictionMatches, visibleSegments, onStep, onShowAll }: { trace: TraceResult; prediction: string; predictionMatches: boolean; visibleSegments: number; onStep: () => void; onShowAll: () => void }) {
  return <section className="trace-record" aria-live="polite" aria-labelledby="trace-title"><h2 id="trace-title">관찰 기록 · 텍스트 경로표</h2><div className="compare-record"><p><strong>첫 예상</strong><span>{prediction}</span></p><p><strong>관찰 결과</strong><span>{trace.summary}</span></p></div><p className="comparison">{predictionMatches ? "예상과 관찰이 같았어요. 어느 장치에서 빛길이 바뀌었는지 근거로 설명해 봐요." : "예상과 관찰을 비교해 봐요. 실제 빛길이 멈추거나 바뀐 지점이 수정의 근거예요."}</p><div className="trace-controls"><button type="button" onClick={onStep} disabled={visibleSegments >= trace.segments.length}>한 단계씩</button><button type="button" onClick={onShowAll}>전체 경로 보기</button></div><ol>{trace.segments.map((segment) => <li key={segment.label}>{segment.label}</li>)}</ol></section>;
}

function SessionSummary({ records, onRestart }: { records: MissionRecord[]; onRestart: () => void }) {
  const missions = mainMissionRecords(records);
  return <section className="summary" aria-labelledby="summary-title"><p className="eyebrow">빛길 관찰 기록</p><h1 id="summary-title">예상하고, 관찰하고, 근거로 설명했어요.</h1><p>점수 대신 내가 확인한 빛의 성질을 다시 살펴봐요.</p><div className="summary-table" role="table" aria-label="미션별 관찰 기록"><div role="row" className="table-head"><span role="columnheader">활동</span><span role="columnheader">첫 예상·최종 선택</span><span role="columnheader">관찰·근거</span></div>{missions.map((record) => <div role="row" key={record.id}><span role="cell">{getMission(record.id).property}</span><span role="cell">{record.prediction}<br />{record.setup}<br />{record.attempts}회 확인 · {record.changed ? "선택을 고쳐 봄" : "첫 선택 유지"}</span><span role="cell">{record.trace.summary}<br />{record.explanation}</span></div>)}</div><section className="next-step"><h2>실제 실험에서 확인할 점</h2><p>화면의 선은 모형 표시예요. 실제 거울과 렌즈 관찰은 선생님의 안내에 따라 안전하게 진행해요.</p></section><button className="primary-button" type="button" onClick={onRestart}>처음부터 다시 살펴보기</button></section>;
}
