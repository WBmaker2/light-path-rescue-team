...
export function repairHintFor(trace: TraceResult, attempts: number) {
  const statusGuide = trace.status === "blocked" ? trace.segments.length === 0 ? "빛길이 시작되지 않았어요. 광원이 켜져 있는지 확인하고" : "빛길이 끝까지 이어지지 않았어요. 막힌 곳을 확인하고" : trace.status === "mirror-back" ? "빛이 거울 뒷면에 닿았어요. 거울 방향을 확인하고" : trace.status === "out-of-bounds" ? "빛길이 표적과 다른 방향으로 갔어요. 위치나 방향을 확인하고" : trace.status === "focus-before-target" ? "빛이 표적보다 앞에서 모였어요. 렌즈 위치를 확인하고" : trace.status === "focus-after-target" ? "빛이 표적보다 뒤에서 모였어요. 렌즈 위치를 확인하고" : "빛길을 다시 확인하고";
